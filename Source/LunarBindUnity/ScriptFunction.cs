namespace LunarBind
{
    ...
    //This is the unity version of the file
    /// <summary>
    ...
    public class ScriptFunction
    {
        ...
        public virtual void ResetCoroutine()
        {
            if (IsCoroutine)
            {
                ...
                {
                    if (CoroutineState == CoroutineState.Dead)
                    {
                        Coroutine = ScriptRef.RecycleCoroutine(Coroutine.Coroutine, LuaFunc);
                    }
                    else
                    {
                        Coroutine = ScriptRef.CreateCoroutine(LuaFunc);
        ...
        public DynValue Execute(params object[] args)
        {
                ...
                if (IsCoroutine)
                {
                    ...
                    switch (co.State)
                    {
                        case CoroutineState.Suspended:
                            if (ret.Type == DataType.UserData)
                            {
                                var retObj = ret.UserData.Object;
                                if (retObj is Yielder yielder)
                                {
                                    CurYielder = yielder;
                                }
                                else if (retObj is IEnumerator enumerator)
                                {
                                    CurYielder = UnityHelper.CoroutineRunner.WaitForCoroutine(enumerator);
                                }
                                //else ignore it
                            }
                            break;
            ...
            {
                //For unity since it is not displayed properly
            ...
            if (IsCoroutine)
            {
                ...
                switch (Coroutine.Coroutine.State)
                {
                    ...
                    case CoroutineState.Dead:
                        ...
                        if (AutoResetCoroutine)
                        {
                            ResetCoroutine();// Coroutine = ScriptRef.RecycleCoroutine(Coroutine.Coroutine, LuaFunc);
                        }
                        break;
                    default:
                        break;
        ...
        public IEnumerator CloneUnityCoroutine(params object[] args)
        {
            ...
            else
            {
                ...
                var func = new ScriptFunction(this, false);
        ...
        /// <summary>
        /// Returns an IEnumerator with this ScriptFunction that yield returns null every frame. 
        /// If it is a normal function it will call once and yield break.<para/>
        /// The <see cref="Yielder"/> attached to the ScriptFunction is checked every iteration
        /// </summary>
        /// <param name="callback">The action to be called when the coroutine is completed</param>
